...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#filter-penyedia').on('change', function() {
            ...
            if (filterPenyedia) {
                $('#filter-layanan').select2({
                    ...
                    allowClear: true,
                    ajax: {
                        url: `{{ url('/api/insiden-layanan/get-layanan') }}/${filterPenyedia}`,
                        dataType: 'json',
                        delay: 250,
                        data: params => ({
                            q: params.term
        ...
        function loadData(page = 1) {
            $.ajax({
                url: "{{ url('/api/insiden-layanan/get-insiden-layanan') }}",
                method: "GET",
                data: {
                    page,
                    per_page: perPage,
                ...
                error: function(xhr) {
                    console.error(xhr);
                    Swal.fire("Error", "Gagal memuat data insiden layanan.", "error");
                }
            });
        }
        ...
        function loadSummary() {
            $.ajax({
                url: "{{ url('/api/insiden-layanan/summary') }}",
                method: "GET",
                success: function(res) {
                    $("#summary-dalam-antrian").text(res.dalam_antrian);
                    $("#summary-verifikasi").text(res.penugasan);
                    $("#summary-proses").text(res.penanganan);
