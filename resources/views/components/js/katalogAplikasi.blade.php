<script>
    document.addEventListener("alpine:init", () => {
        Alpine.store('tableStore', {
            ...
            async loadData() {
                const res = await fetch("{{ url('/api/katalog-aplikasi/get-katalog-aplikasi') }}");
                ...
                this.datatable = new simpleDatatables.DataTable('#myTable2', {
                    data: {
                        headings: ["No", "Kode", "Nama Aplikasi", "Nama PPK", "Perangkat Daerah", "No Rekomendasi", "No Pelaporan", "No Pengujian", "id"],
                        data: result.data
                    },
                    searchable: true,
                    perPage: 10,
    ...
    //         try {
    //             const res = await fetch(`{{ url('/api/perangkat-daerah') }}/${hashedId}/destroy`, {
    //                 method: 'DELETE',
    //                 headers: {
    ...
    //             } else {
    //                 Swal.fire("Gagal!", data.message || "Gagal mengambil data perangkat daerah.", "error");
    //             }
    //         })
