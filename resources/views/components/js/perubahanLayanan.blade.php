...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#filter-penyedia').on('change', function() {
            ...
            if (filterPenyedia) {
                $('#filter-layanan').select2({
                    ...
                    allowClear: true,
                    ajax: {
                        url: `{{ url('/api/perubahan-layanan/get-layanan') }}/${filterPenyedia}`,
                        dataType: 'json',
                        delay: 250,
                        data: params => ({
                            q: params.term
        ...
        function loadData(page = 1) {
            $.ajax({
                url: "{{ url('/api/perubahan-layanan/get-perubahan-layanan') }}",
                method: "GET",
                data: {
                    page,
                    per_page: perPage,
                ...
                error: function(xhr) {
                    console.error(xhr);
                    Swal.fire("Error", "Gagal memuat data perubahan layanan.", "error");
                }
            });
        }
        ...
        function loadSummary() {
            $.ajax({
                url: "{{ url('/api/perubahan-layanan/summary') }}",
                method: "GET",
                success: function(res) {
                    $("#summary-dalam-antrian").text(res.dalam_antrian);
                    $("#summary-verifikasi").text(res.disetujui);
                    $("#summary-proses").text(res.dilaksanakan);
                    $("#summary-penelusuran").text(res.penelusuran);
                    $("#summary-selesai").text(res.selesai);
                    $("#summary-ditolak").text(res.ditolak);
                    $("#summary-pelaporan").text(res.pelaporan);
                }
            });
        }
