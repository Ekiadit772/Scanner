<x-app-layout>
    ...
    <div class="panel lg:row-span-1 mt-8">
        <div class="mb-5">
            <form id="formService" class="space-y-5 px-0 sm:px-10" enctype="multipart/form-data">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center mb-3">
                    <label for="penyedia-layanan" class="sm:w-44 text-sm font-medium">Nama Bidang</label>
                    <div class="flex-1">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center mb-3">
                    <label for="keterangan_status" class="sm:w-44 text-sm font-medium">Komentar (Opsional)</label>
                    <div class="flex-1">
                        <textarea id="keterangan_status" class="form-textarea border rounded p-2" name="keterangan_status"></textarea>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-2 mt-6">
                    ...
                    @if ($currentStatus === 'dalam antrian')
                        <button type="button" onclick="updateStatus('{{ $hashedAgain }}', 'verifikasi')"
    ...
    <script>
        async function updateStatus(hashedAgain, status) {

            const keterangan_status = document.getElementById('keterangan_status').value.trim();

            // Validasi khusus untuk ditolak
            if (status === 'ditolak' && !keterangan_status) {
                Swal.fire("Error!", "Keterangan penolakan wajib diisi.", "warning");
                return;
            }

            try {
                const res = await fetch(`{{ url('/api/katalog-layanan/update-status') }}/${hashedAgain}`, {
                    method: 'POST',
                    headers: {
