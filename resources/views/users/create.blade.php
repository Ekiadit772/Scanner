<x-app-layout>
    ...
    <div class="panel lg:row-span-3">
        ...
        <div class="mb-5">
            <form class="space-y-5" method="POST" action="{{ route('users.store') }}">
                @csrf
                <div>
                    <label for="name">Name</label>
                    <x-text-input id="name" class="block mt-1 w-full" type="name" name="name"
                        :value="old('name')" required autocomplete="username" placeholder="Enter name" />
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>
                <div>
                    <label for="name">Email</label>
                    <x-text-input id="email" class="block mt-1 w-full" type="email" name="email"
                        :value="old('email')" required autocomplete="username" placeholder="Enter Email" />
                ...
                <div>
                    ...
                    <x-text-input id="phone" class="block mt-1 w-full" type="phone" name="phone"
                        :value="old('phone')" required autocomplete="username" placeholder="Enter phone" />
                ...
                <div class="flex justify-between">
                    <a href="{{ route('users.index') }}" class="bg-gray-300 text-black px-4 py-2 rounded hover:bg-gray-400">kembali</a>
